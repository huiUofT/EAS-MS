...
Peakextract <- function(Intensitycut,ppm,folder,Pool,Control,STD) {
  ...
  index3<-grep(STD,msfiles[index2])##the msfiles from STD
  msfiles<-msfiles[c(index1,index2[index3])]#the data of STD and controls from the same pool
  ...
  xset2<-Fillpeak(xset1,10,30,msfiles,Intensitycut)
  ...
  #'reevaluate ms peaks
  Library.new<-mzSmooth(Allpeak,msfiles,45,5)
...
#'
Fillpeak<-function(xset,ppm,btw,msfiles,Intensitycut){
  ppm<-ppm/10^6
  xset.input<-xset
  ...
  for (i in 1:length(xset.input@groupidx)){
    ...
    intensity<-xset.input@peaks[index,9]
    index1<-which(intensity>Intensitycut)##only care about high intensity
